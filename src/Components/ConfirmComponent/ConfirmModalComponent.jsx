...
function ConfirmModalComponent({ show, setShow }) {
    ...
    const { orderList, setOrderList, totalMoney, tax, userInfo } = useContext(Context);
    const createCart = async (userInfo) => {
        userInfo.productItems = orderList;
        userInfo.totalPrice = totalMoney + tax;
        const result = await axios.post("https://65784f07f08799dc8044df82.mockapi.io/cartList", userInfo);
    };
            ...
            <Modal scrollable={true} show={show} backdrop="static" keyboard={false}>
                ...
                {isConfirm === false ? (
                    <>
                        <Modal.Body>
                            <ListGroup>
                                <ListGroup.Item>Tên: {userInfo.fullName}</ListGroup.Item>
                                <ListGroup.Item>Số điện thoại: {userInfo.phoneNumber}</ListGroup.Item>
                                <ListGroup.Item>
                                    Địa chỉ giao hàng:{" "}
                                    {userInfo.street + ", " + userInfo.wardValue + ", " + userInfo.districtValue + ", " + userInfo.cityValue}
                                </ListGroup.Item>
                                    ...
                                    <Table borderless className="text-center">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{(totalMoney + tax).toLocaleString("vi") + " ₫"}</td>
                                            </tr>
                                        </tbody>
                                    </Table>
                                </div>
                        ...
                        <Modal.Body>
                            <div className="process-container">
                                <div className="loadingThumb">
                                    <img style={{ width: "200px", height: "200px", margin: "0px auto" }} src={logo} alt="" />
                                </div>
                                <p className="text-center">Đang xữ lý...</p>{" "}
                            </div>
                            <div className="confirm-container text-center py-5">
                                <h4 className="confirm-title">Vuông tròn đã ghi nhận thông tin của quý khách.</h4>
                                <p className="confirm-thank">Cảm ơn quý khách hàng đã lựa chọn Cơm niêu Vuông tròn.</p>
                            </div>
                        </Modal.Body>
                    </>
                )}
